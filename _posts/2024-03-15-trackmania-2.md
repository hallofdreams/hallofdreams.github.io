...
> Here's a step-Running a graphical application like Trackmania on a Linux server in a headless session can be a bit tricky, but it's possible to achieve with a combination of Xvfb (X Virtual Framebuffer) and a VNC server.  Here's a step-by-step guide to accomplish this:
