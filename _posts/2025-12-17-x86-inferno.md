...
<div style="display:none;">A descent into the madness of solving Advent of Code 2025 in x86 assembly, featuring manual memory management, syscalls, and rage sorting.</div>
...
When I wrote Python first and then translated line by line, I wasn't solving the problem slowly. I was doing two completely different things in sequence: thinking about the problem and expressing computation to a machine. I had to do them serially because they couldn't fit in my head at the same time. Just as with computers, my working memory is finite. When it's occupied by "r12 has the loop counter, rcx will get clobbered by div, I need to preserve it, where did I put the base pointer" there's no room left for "what's the recursive structure of this packing problem?" The registers don't make Advent of Code harder. They replace Advent of Code with a different problem entirely, one about expression rather than solution. Every layer of abstraction is a bet: this is safe to forget. When you write Ruby, you're trusting that memory management isn't where you'll fail. That bet buys you cognitive space to think about the thing you actually meant to think about.

Strip the abstraction away and you discover the real shape of hell: it's not that the work is tedious. It's that it captures your attention so completely that the problem you came to solve becomes unreachable. The torment isn't the registers. The torment is that the registers are all you can see. Sometimes an abstraction is bad because it unintentionally adds a cost you didn't expect. Most of the time it's just saving you from the hubris of thinking you know better than the machine. 
